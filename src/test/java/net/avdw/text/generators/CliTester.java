...
public class CliTester {
    ...
    public CliTester execute(final String command, final String... arguments) {
        ...
        Reflections reflections = new Reflections("net.avdw.text.generators");
        reflections.getTypesAnnotatedWith(CommandLine.Command.class).stream()
                .filter(type -> !MainCli.class.equals(type))
                .forEach(commandLine::addSubcommand);

        commandLine.setOut(new PrintWriter(new AnsiOutputStream(out), true, StandardCharsets.UTF_8));
        commandLine.setErr(new PrintWriter(new AnsiOutputStream(err), true, StandardCharsets.UTF_8));
        if (command == null) {
